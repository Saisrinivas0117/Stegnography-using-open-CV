        ...
        for pixel in i:
            ...
            for byte in total_bytes:
                ...
                if decoded_data[-5:] == "*^*^*":
                    ...
                    return
def encode_vid_data():
    cap=cv2.VideoCapture(input("Enter the name of the Video file in which you want to Encode data :"))
    ...
    out = cv2.VideoWriter(input("\nEnter the name of the Stego video after Encoding(with extension):"),fourcc, 25.0, size)
    frame_number = 0
    while(cap.isOpened()):
        frame_number += 1
        ret, frame = cap.read()
